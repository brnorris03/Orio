void VecNorm2(int n, double *x, double r) {
    ...
    ) @*/
    {
      ...
      for (istream=0; istream<nstreams; istream++ ) {
        ...
        orcu_kernel4<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunklen,dev_x+soffset,dev_r+boffset);
      }
      if (chunkrem!=0) {
        soffset=istream*chunklen;
        boffset=istream*blks4chunk;
        orcu_kernel4<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunkrem,dev_x+soffset,dev_r+boffset);
        blks4chunks++ ;
      }
