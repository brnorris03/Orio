void VecDot(int n, double *x, double *y, double r) {
  ...
  ) @*/
  {
    ...
    for (istream=0; istream<nstreams; istream++ ) {
      ...
      orcu_kernel3<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunklen,dev_y+soffset,dev_x+soffset,dev_r+boffset);
    }
    if (chunkrem!=0) {
      soffset=istream*chunklen;
      boffset=istream*blks4chunk;
      orcu_kernel3<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunkrem,dev_y+soffset,dev_x+soffset,dev_r+boffset);
      blks4chunks++ ;
    }
...
__global__ void orcu_kernel3(const int n, double* y, double* x, double* reducts) {
