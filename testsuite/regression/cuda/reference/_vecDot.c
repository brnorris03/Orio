void VecDot(int n, double *x, double *y, double r) {
    ...
    {
      /*declare variables*/
      double *dev_y, *dev_x, *dev_reducts;
      int nthreads=16;
      int nstreams=2;
      ...
      for (orcu_i=0; orcu_i<nstreams; orcu_i++ ) {
        ...
        orcu_kernel4<<<blks4chunk,dimBlock,0,stream[orcu_i]>>>(chunklen,dev_y+orcu_soff,dev_x+orcu_soff,dev_reducts+orcu_boff);
      }
      if (chunkrem!=0) {
        orcu_soff=orcu_i*chunklen;
        orcu_boff=orcu_i*blks4chunk;
        orcu_kernel4<<<blks4chunk,dimBlock,0,stream[orcu_i]>>>(chunkrem,dev_y+orcu_soff,dev_x+orcu_soff,dev_reducts+orcu_boff);
        blks4chunks++ ;
      }
