void VecSum(int n, double *x, double s) {
  ...
  ) @*/
  {
    ...
    for (istream=0; istream<nstreams; istream++ ) {
      ...
      orcu_kernel3<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunklen,dev_x+soffset,dev_s+boffset);
    }
    if (chunkrem!=0) {
      soffset=istream*chunklen;
      boffset=istream*blks4chunk;
      orcu_kernel3<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunkrem,dev_x+soffset,dev_s+boffset);
      blks4chunks++ ;
    }
    ...
    while (orcu_blks>1) {
      ...
      orcu_blksum4<<<orcu_blks,32>>>(orcu_n,dev_s);
    }
