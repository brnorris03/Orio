void VecAXPY(int n, double a, double *x, double *y) {
    ...
    ) @*/
    {
      ...
      for (istream=0; istream<nstreams; istream++ ) {
        soffset=istream*chunklen;
        orcu_kernel3<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunklen,a,dev_y+soffset,dev_x+soffset);
      }
      if (chunkrem!=0) {
        soffset=istream*chunklen;
        orcu_kernel3<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunkrem,a,dev_y+soffset,dev_x+soffset);
      }
...
__global__ void orcu_kernel3(int n, double a, double* y, double* x) {
