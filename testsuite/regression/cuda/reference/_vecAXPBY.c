void VecAXPBY(int n, double a, double *x, double b, double *y) {
    ...
    ) @*/
    {
      ...
      for (istream=0; istream<nstreams; istream++ ) {
        soffset=istream*chunklen;
        orcu_kernel3<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunklen,a,b,dev_y+soffset,dev_x+soffset);
      }
      if (chunkrem!=0) {
        soffset=istream*chunklen;
        orcu_kernel3<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunkrem,a,b,dev_y+soffset,dev_x+soffset);
      }
...
__global__ void orcu_kernel3(int n, double a, double b, double* y, double* x) {
