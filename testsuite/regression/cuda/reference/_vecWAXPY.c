void VecWAXPY(int n, double *w, double a, double *x, double *y) {
  ...
  ) @*/
  {
    ...
    for (istream=0; istream<nstreams; istream++ ) {
      soffset=istream*chunklen;
      orcu_kernel2<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunklen,a,dev_x+soffset,dev_y+soffset,dev_w+soffset);
    }
    if (chunkrem!=0) {
      soffset=istream*chunklen;
      orcu_kernel2<<<blks4chunk,dimBlock,0,stream[istream]>>>(chunkrem,a,dev_x+soffset,dev_y+soffset,dev_w+soffset);
    }
...
__global__ void orcu_kernel2(const int n, double a, double* x, double* y, double* w) {
  const int tid=blockIdx.x*blockDim.x+threadIdx.x;
  const int gsize=gridDim.x*blockDim.x;
  __shared__ double shared_y[32];
  __shared__ double shared_x[32];
  __shared__ double shared_w[32];
