...
__global__ void multMM_kernel(double*,double*,double*);
double getclock();
...
//	allocate random MatrixA 
	...
	double* MatrixA=(double*) malloc(memsizeA*sizeof(double)); 
	printf("\n\nmemsize of MatrixA: %luMB\n",(sizeof(double)*MATRIX_DIM_M*MATRIX_DIM_N)/1000000);

//	allocate random MatrixB 
	unsigned int memsizeB=MATRIX_DIM_N*MATRIX_DIM_O;
	double* MatrixB=(double*) malloc(memsizeB*sizeof(double)); 
	printf("memsize of MatrixB: %luMB\n",(sizeof(double)*memsizeB)/1000000);

//	allocate empty returnMatrix 
	unsigned int memsizeC=MATRIX_DIM_M*MATRIX_DIM_O;
	double* returnMatrix=(double*) malloc(memsizeC*sizeof(double)); 
	printf("memsize of returnMatrix: %luMB\n",(sizeof(double)*memsizeC)/1000000);

	printf("Total memsize of matrices: %luMB\n\n",(sizeof(double)*(memsizeA+memsizeB+memsizeC))/1000000);


//	fill Matrices
	for(i=0;i<MATRIX_DIM_M;i++){
           for(j=0;j<MATRIX_DIM_N;j++)
	       MatrixA[j*MATRIX_DIM_M+i]=(double) 1.0*(1.0/((rand()%100)+1));    	       
	}

	for(i=0;i<MATRIX_DIM_N;i++){
           for(j=0;j<MATRIX_DIM_O;j++)
	       MatrixB[j*MATRIX_DIM_N+i]=(double) 1.0*(1.0/((rand()%100)+1));    	       
	}
...
// 	allocate GPU device memory
	cs=getclock();
	double* dev_MatrixA;
	cudastatus0=cudaMalloc((void**)&dev_MatrixA,MATRIX_DIM_M*MATRIX_DIM_N*sizeof(double));
	cudastatus1=cudaMemcpy(dev_MatrixA,MatrixA,MATRIX_DIM_M*MATRIX_DIM_N*sizeof(double),cudaMemcpyHostToDevice);
	...
	double* dev_MatrixB;
	cudastatus2=cudaMalloc((void**)&dev_MatrixB,MATRIX_DIM_N*MATRIX_DIM_O*sizeof(double));
	cudastatus3=cudaMemcpy(dev_MatrixB,MatrixB,MATRIX_DIM_N*MATRIX_DIM_O*sizeof(double),cudaMemcpyHostToDevice);
	...
	double* dev_returnMatrix;
	cudastatus4=cudaMalloc((void**)&dev_returnMatrix,MATRIX_DIM_M*MATRIX_DIM_O*sizeof(double));
	cudastatus5=cudaMemset(dev_returnMatrix,0.0,MATRIX_DIM_M*MATRIX_DIM_O*sizeof(double));
...
//	kernel time
	...
	cudastatus7=cudaMemcpy(returnMatrix,dev_returnMatrix,
		MATRIX_DIM_M*MATRIX_DIM_O*sizeof(double),cudaMemcpyDeviceToHost);
...
*/
	...
	cs = getclock();
	double sum =0.;
	double* MatrixC = (double*) calloc(MATRIX_DIM_M*MATRIX_DIM_O,sizeof(double));
	...
	//	printf("GPUmatrix: %f\n",returnMatrix[i]);
		if (answer>0.00001){	
		   printf("Error, divergent value: %3.12lf for index: %d\n",answer,i);
		   printf("CPUmatrix element (serial): %3.12lf\n",MatrixC[i]);
		   printf("GPUmatrix element: %3.12lf\n",returnMatrix[i]);
		   counter++;
...
__global__ void multMM_kernel(double* A, double* B, double* C){
