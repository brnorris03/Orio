...
__global__ void multMV_kernel(float*,float*,float*);
double getclock();
...
//	allocate random Matrix 
	unsigned int memmatrixsize=sizeof(float)*MATRIX_DIM_M*MATRIX_DIM_N;
	float* Matrix=(float*) malloc(memmatrixsize); 
	printf("\n\nmemsize of Matrix: %luKB\n",memmatrixsize/1000);

//      Dense vector
	unsigned int memvectorsize=MATRIX_DIM_N*sizeof(float);
	float* vector=(float*)malloc(memvectorsize);
	printf("memsize of vector: %luMB\n",memvectorsize/1000);
	printf("Total data transfer to gpu: %luKB\n",(memvectorsize+memmatrixsize)/1000);

	float* returnvector=(float*)malloc(MATRIX_DIM_M*sizeof(float));

//	fill Matrix and vector
//	#pragma omp parallel for private(i,j)
	for(i=0;i<MATRIX_DIM_M;i++){
           
           for(j=0;j<MATRIX_DIM_N;j++) Matrix[j*MATRIX_DIM_M+i]=(float) 1.0*(1.0/((rand()%100)+1));    
	}
	for(i=0;i<MATRIX_DIM_N;i++)
		vector[i]=(float) 1.0*(1.0/((rand()%100)+1))+perturb;
...
// 	allocate GPU device memory
	cs=getclock();
	float* dev_vector;
	cudastatus0=cudaMalloc((void**)&dev_vector,MATRIX_DIM_N*sizeof(float));
	cudastatus1=cudaMemcpy(dev_vector,vector,MATRIX_DIM_N*sizeof(float),cudaMemcpyHostToDevice);
	...
	float* dev_Matrix;
	cudastatus2=cudaMalloc((void**)&dev_Matrix,MATRIX_DIM_M*MATRIX_DIM_N*sizeof(float));
	cudastatus3=cudaMemcpy(dev_Matrix,Matrix,MATRIX_DIM_M*MATRIX_DIM_N*sizeof(float),cudaMemcpyHostToDevice);
	...
	float* dev_returnVector;
	cudastatus4=cudaMalloc((void**)&dev_returnVector,MATRIX_DIM_M*sizeof(float));
	cudastatus5=cudaMemset(dev_returnVector,0.0,MATRIX_DIM_M*sizeof(float));
...
//	kernel time
	...
	cudastatus7=cudaMemcpy(returnvector,dev_returnVector,MATRIX_DIM_M*sizeof(float),cudaMemcpyDeviceToHost);
	...
	cs = getclock();
	float* wvector = (float*) calloc(MATRIX_DIM_M,sizeof(float));
	#pragma omp parallel for private(i,j)
	for(i=0;i<MATRIX_DIM_M;i++){
	...
	free(wvector);
	cs = getclock();
	wvector = (float*) calloc(MATRIX_DIM_M,sizeof(float));
	for(i=0;i<MATRIX_DIM_M;i++){
		for(j=0;j<MATRIX_DIM_N;j++){
			wvector[i]+=Matrix[i*MATRIX_DIM_N+j]*vector[j];
		}
...
// Y-BLOCK TILES
__global__ void multMV_kernel(float* M, float* v, float* w){

	unsigned int i,j,tileId,offset,vindex,ypos;
	__shared__ float vcache[THREADS];
	__shared__ float wback[THREADS];
