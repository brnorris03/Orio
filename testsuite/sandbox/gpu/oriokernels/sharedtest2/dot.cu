...
//	some general cpu info
	...
	cudaError_t cudastatus0,cudastatus1;
        ...
        cudastatus0=cudaDeviceSetLimit(cudaLimitMallocHeapSize, 128*1024*sizeof(float));
        if(cudastatus0!=cudaSuccess) printf("Error in heap memory set, status: %s\n",
	...
	cudaEventRecord(start2,0);//begin recording kernel
  	doto<<<dimGrid,dimBlock,sharesize>>>(devX,devY,devZ,devLen);
...
__global__ void doto(float* x, float* y, float* z, int* vlength){
	...
	int tid = blockDim.x*blockIdx.x+threadIdx.x;//unneeded in new code
	
	int offset=blockDim.x/2;// modified var
