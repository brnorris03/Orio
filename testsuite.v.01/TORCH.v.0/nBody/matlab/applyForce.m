function particle = applyForce(particle, neighbor, cutoff, min_r, type)
%
...
% 
    ...
    if( type ~= 1)
        ...
        if distance <= .001
            ...
            force = (G * particle.mass * neighbor.mass) / .001^2 ; 
            particle.ax = particle.ax + dx * force; 
            particle.ay = particle.ay + dy * force;
        else
            force = (G * particle.mass * neighbor.mass) / distance^2 ; 
            particle.ax = particle.ax + dx * force; 
            particle.ay = particle.ay + dy * force;
        end
