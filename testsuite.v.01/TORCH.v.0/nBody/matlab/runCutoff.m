function [ forceLog ] = runCutoff(n, numSteps, forceType, output, saveFreq, verify) 
%
...
%
    ...
    if (output) 
        file = fopen('cutoff.txt', 'w'); 
        fprintf(file, '%d %g\n', n, size); 
    end
    
    particles = initParticles(n, size, mass); 
    ...
    for step = 1:numSteps
            ...
            for i = 1:bins(k).numParticles
                for j = 1:bins(k).numParticles
                    ...
                    if localIndex ~= neighborIndex
                        particles(localIndex) = applyForce(particles(localIndex), particles(neighborIndex) , cutoff, min_r, forceType); 
                    end
                end
            end
        ...
        %move particles
        for i = 1:n
            particles(i) = move(particles(i), size, dt, forceType); 
        end
        
        if (output) && (mod(step, saveFreq) == 0)
            writeToFile(file, n, particles) ;
        end
