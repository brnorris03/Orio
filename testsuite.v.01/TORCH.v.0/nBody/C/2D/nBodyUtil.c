...
node * quadTreeBuild(particle_t *particles, int n){
	/*
	 Builds an adaptive quadtree according to the given distribution of particles.
	...
	*rootPtr = computeCenterAndRadius(*rootPtr, particles, n);
	...
	int j;
	for(j=0; j<n; j++){
		*rootPtr = quadTreeInsert(particles, j, *rootPtr);
	}
...
node quadTreeInsert(particle_t *particles, int j, node root){
	/*
	 Inserts a particle into an adaptive quadtree.
	...
	if(root.isInternal){  // root has 4 children
		...
		root.children[index] = quadTreeInsert(particles, j, root.children[index] );
	}
...
node computeCMandTM(node nn, particle_t *particles){
	...
	if(nn.particleIndex != -1){  // if n is a leaf
		...
		nn.totalMass =  nn.children[0].totalMass +
						nn.children[1].totalMass +
						nn.children[2].totalMass +
						nn.children[3].totalMass ;

		if(nn.totalMass == 0.0){ // should never occur if recursion works properly
			fprintf(stderr, "WARNING: Big problems. Zero mass on an internal node.\n") ;
		}

		// each coordinate of center of mass is mass weighted sum of all children's
		nn.centerMass[0] = (nn.children[0].totalMass * nn.children[0].centerMass[0] +
						  nn.children[1].totalMass * nn.children[1].centerMass[0] +
						  nn.children[2].totalMass * nn.children[2].centerMass[0] +
						  nn.children[3].totalMass * nn.children[3].centerMass[0] ) / nn.totalMass;

		nn.centerMass[1] = (nn.children[0].totalMass * nn.children[0].centerMass[1] +
						  nn.children[1].totalMass * nn.children[1].centerMass[1] +
						  nn.children[2].totalMass * nn.children[2].centerMass[1] +
						  nn.children[3].totalMass * nn.children[3].centerMass[1] ) / nn.totalMass;
...
char verifyNBody(forceLog guess, forceLog true, double rmsErrorTol){
	...
	double currentErrX, currentErrY ;
	double maxErrX = 0.0 ;
	double maxErrY = 0.0 ;
