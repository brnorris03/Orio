...
void runNaive(int n, int numSteps, int forceType, char output, int saveFreq, char verify, forceLog currentLog){
	/*
	 ...
	 Input:
	     ...
	     forceLog currentLog    Structure for logging forces. Must be preallocated if verification is on.
	...
	FILE *fsave = fopen( "naive3d.txt", "w" );
    ...
    for(step = 0; step < numSteps; step++ ){
        ...
        if( verify && (step < currentLog.steps) ){
        	for(i=0; i<currentLog.length; i++){
        		...
        		currentLog.zForces[step][i] = particles[i].az * particles[i].mass ;
        	}
        }
...
void runCutoff(int n, int numSteps, int forceType, char output, int saveFreq, char verify, forceLog currentLog){
	...
	FILE *fsave = fopen( "cutoff3d.txt", "w" );
...
void runBarnesHut(int n, int numSteps, int forceType, char output, int saveFreq, char verify, forceLog currentLog){
	...
	FILE *fsave = fopen( "barnesHut3d.txt", "w" );
	...
	for (step = 0; step < numSteps; step++) {
        ...
        //  compute forces
    	octTree = octTreeBuild(particles, n);
    	*octTree = computeCMandTM(*octTree, particles);
        ...
        for(i = 0; i < n; i++ ){
        	...
        	particles[i].ay = 0.0;
			particles[i].az = 0.0;
        	treeForce(particles, i, *octTree, forceType);
        }
...
void apply_force( particle_t *particle, particle_t *neighbor, int type){
	...
	if(type == 0 || type > 1){
	    ...
	    if (distance <= 0.001){
	        force = (G * particle->mass * neighbor->mass) / (0.001 * 0.001 * 0.001) ;
			particle->ax += dx * force;
			particle->ay += dy * force;
			particle->az += dy * force;

	    }
	    else{
	        force = (G * particle->mass * neighbor->mass) / (distance * distance * distance) ;
	        particle->ax += dx * force;
	        particle->ay += dy * force;
			particle->az += dz * force;
	    }
	}
...
void treeForce( particle_t *particles, int j, node nn, int forceType ){
	...
	if(nn.particleIndex != -1){ // if node has a particle, interact with it
		...
		double dz = nn.centerMass[2] - particles[j].z ;
		double distance = sqrt(dx*dx + dy*dy + dz*dz);
		...
		if( ((2.0*nn.radius) / distance) < THETA ){
			...
			if( cutoffAllowed ){
				if (distance <= 0.001){
					double force = 9.8 * particles[j].mass * nn.totalMass / (0.001 * 0.001 * 0.001) ;
					particles[j].ax += dx * force;
					particles[j].ay += dy * force;
					particles[j].az += dz * force;
				}
				else{
					double force = 9.8 * particles[j].mass * nn.totalMass / (distance * distance * distance) ;
					particles[j].ax += dx * force;
					particles[j].ay += dy * force;
					particles[j].az += dz * force;
				}
			...
			int i;
			for(i=0; i<8; i++){
				treeForce(particles, j, nn.children[i], forceType) ;
			}
		}
