...
if (N >= 2) {
  for (c1=0;c1<=floord(N-2,256);c1++) {
    for (c2=max(0,ceild(128*c1-255,256));c2<=floord(N-1,512);c2++) {
      for (c3=max(ceild(128*c1-127,128),ceild(128*c1-32385,32640));c3<=floord(N-1,256);c3++) {
        for (c4=max(max(16*c1-3840*c3-3810,0),16*c1);c4<=min(min(min(min(floord(N-2,16),16*c1+15),floord(128*c3+127,8)),floord(256*c2+255,8)),floord(1920*c3+1905,8));c4++) {
          for (c6=max(max(max(max(ceild(-16*c1+16*c3+c4-225,241),ceild(16*c1-16*c3-c4-225,239)),ceild(8*c4-105,120)),16*c3),ceild(8*c4-7,8));c6<=min(16*c3+15,floord(N-1,16));c6++) {
            if ((c1 == c3) && (c4 == c6)) {
              for (c7=max(0,16*c6);c7<=min(min(N-2,512*c2+510),16*c6+14);c7++) {
                for (c8=max(512*c2,c7+1);c8<=min(512*c2+511,N-1);c8++) {
...
transform Composite(
permut = [['c9', 'c7', 'c8']],
  regtile = (['c7', 'c8', 'c9'],[7, 1, 5]),
  scalarreplace = (True, 'double'),
  vector = (True, ['ivdep','vector always']))

            for (c7=max(0,16*c4);c7<=min(min(16*c6-1,512*c2+510),16*c4+15);c7++) {
              for (c8=max(512*c2,c7+1);c8<=min(N-1,512*c2+511);c8++) {
  ...
  for (c9t=16*c6; c9t<=min(N-1,16*c6+15)-4; c9t=c9t+5) {
    for (c7t=max(0,16*c4); c7t<=min(min(16*c6-1,512*c2+510),16*c4+15)-6; c7t=c7t+7) {
      for (c7=c7t; c7<=c7t+6; c7=c7+1) {
        register int cbv_1, cbv_2;
        cbv_1=max(512*c2,c7+1);
        cbv_2=min(N-1,512*c2+511);
#pragma ivdep
#pragma vector always
    ...
    for (c7=c7t; c7<=min(min(16*c6-1,512*c2+510),16*c4+15); c7=c7+1) {
      register int cbv_3, cbv_4;
      cbv_3=max(512*c2,c7+1);
      cbv_4=min(N-1,512*c2+511);
#pragma ivdep
#pragma vector always
  ...
  for (c9=c9t; c9<=min(N-1,16*c6+15); c9=c9+1) {
    for (c7t=max(0,16*c4); c7t<=min(min(16*c6-1,512*c2+510),16*c4+15)-6; c7t=c7t+7) {
      for (c7=c7t; c7<=c7t+6; c7=c7+1) {
        ...
        cbv_5=max(512*c2,c7+1);
        cbv_6=min(N-1,512*c2+511);
#pragma ivdep
#pragma vector always
    ...
    for (c7=c7t; c7<=min(min(16*c6-1,512*c2+510),16*c4+15); c7=c7+1) {
      ...
      cbv_7=max(512*c2,c7+1);
      cbv_8=min(N-1,512*c2+511);
#pragma ivdep
#pragma vector always
...
/*@ end @*/
            ...
            if ((c1 == c3) && (c2 >= ceild(16*c4-495,512)) && (-c4 == -c6) && (c4 <= floord(N-17,16))) {
              for (c8=max(16*c4+16,512*c2);c8<=min(N-1,512*c2+511);c8++) {
