...
# syntactical error
def t_error(t):
    print 'error:OrTilDriver:%s: syntactical error: "%s"' % (getLineNumber(t.lineno), t.value[0])
    sys.exit(1)
...
# grammatical error
def p_error(p):
    print 'error:OrTilDriver:%s: grammatical error: "%s"' % (getLineNumber(p.lineno), p.value)
    sys.exit(1)
