...
# syntactical error
def t_error(t):
    err('module.ortildriver.code_parser: OrTilDriver:%s: syntactical error: "%s"' % (getLineNumber(t.lineno), t.value[0]))
...
# grammatical error
def p_error(p):
    err('module.ortildriver.code_parser: OrTilDriver:%s: grammatical error: "%s"' % (getLineNumber(p.lineno), p.value))
