...
__start_line_no = None

def getLineNumber(line_no):
    '''Calculate the actual line number in the code, with respect to the starting line number'''
    if __start_line_no == None:
        return ''
    else:
        return line_no + __start_line_no - 1
...
# syntactical error
def t_error(t):
    err('orio.module.ortildriver.code_parser: OrTilDriver:%s: syntactical error: "%s"' % (getLineNumber(t.lineno), t.value[0]))
...
def p_iteration_statement(p):
    ...
    p[0] = ast.ForStmt(p[3], p[5], p[7], p[9], getLineNumber(p.lineno(1)))
...
def p_expression_2(p):
    ...
    p[0] = ast.BinOpExp(p[1], p[3], ast.BinOpExp.COMMA, getLineNumber(p.lineno(1)))
...
def p_assignment_expression_2(p):
    ...
    if (p[2] == '='):
        p[0] = ast.BinOpExp(p[1], p[3], ast.BinOpExp.EQ_ASGN, getLineNumber(p.lineno(1)))
    elif p[2] in ('*=', '/=', '%=', '+=', '-='):
        lhs = p[1].replicate()
        rhs = None
        if (p[2] == '*='):
            rhs = ast.BinOpExp(p[1], p[3], ast.BinOpExp.MUL, getLineNumber(p.lineno(1)))
        elif (p[2] == '/='):
            rhs = ast.BinOpExp(p[1], p[3], ast.BinOpExp.DIV, getLineNumber(p.lineno(1)))
        elif (p[2] == '%='):
            rhs = ast.BinOpExp(p[1], p[3], ast.BinOpExp.MOD, getLineNumber(p.lineno(1)))
        elif (p[2] == '+='):
            rhs = ast.BinOpExp(p[1], p[3], ast.BinOpExp.ADD, getLineNumber(p.lineno(1)))
        elif (p[2] == '-='):
            rhs = ast.BinOpExp(p[1], p[3], ast.BinOpExp.SUB, getLineNumber(p.lineno(1)))
        else:
            err('module.ortildriver.code_parser internal error: unknown assignment operator')
...
def p_logical_or_expression_2(p):
    ...
    p[0] = ast.BinOpExp(p[1], p[3], ast.BinOpExp.LOR, getLineNumber(p.lineno(1)))
...
def p_logical_and_expression_2(p):
    ...
    p[0] = ast.BinOpExp(p[1], p[3], ast.BinOpExp.LAND, getLineNumber(p.lineno(1)))
...
def p_equality_expression_2(p):
    ...
    if p[2] == '==':
        ...
        err('module.ortildriver.code_parser internal error: unknown equality operator')
...
def p_relational_expression_2(p):
    ...
    if (p[2] == '<'):
        ...
        err('module.ortildriver.code_parser internal error: unknown relational operator')
...
def p_additive_expression_2(p):
    ...
    if (p[2] == '+'):
        ...
        err('module.ortildriver.code_parser internal error: unknown additive operator' )
...
def p_multiplicative_expression_2(p):
    ...
    if (p[2] == '*'):
        ...
        err('module.ortildriver.code_parser internal error: unknown multiplicative operator')
...
def p_unary_expression_4(p):
    ...
    if p[1] == '+':
        ...
        err('module.ortildriver.code_parser internal error: unknown unary operator')
...
def p_postfix_expression_2(p):
    ...
    p[0] = ast.ArrayRefExp(p[1], p[3], getLineNumber(p.lineno(1)))
...
def p_postfix_expression_4(p):
    ...
    p[0] = ast.UnaryExp(p[1], ast.UnaryExp.POST_INC, getLineNumber(p.lineno(1)))

def p_postfix_expression_5(p):
    'postfix_expression : postfix_expression MINUSMINUS'
    p[0] = ast.UnaryExp(p[1], ast.UnaryExp.POST_DEC, getLineNumber(p.lineno(1)))
...
def p_primary_expression_4(p):
    ...
    p[0] = ast.StringLitExp(p[1], getLineNumber(p.lineno(1)))
...
# grammatical error
def p_error(p):
    err('orio.module.ortildriver.code_parser: OrTilDriver:%s: grammatical error: "%s"' % (getLineNumber(p.lineno), p.value))

#------------------------------------------------

def getParser(start_line_no = None):
