...
import orio.module.matrix.lexer
...
def setup_regen(debug = 1, outputdir='.'):
    ...
    parsetabfile = os.path.join(os.path.abspath(outputdir),'orio.module.matrix.parsetab.py')
    try: os.remove(parsetabfile)
    except: pass

    parser = orio.tool.ply.yacc.yacc(debug=debug, optimize=1, tabmodule='orio.module.matrix.parsetab', write_tables=1, outputdir=os.path.abspath(outputdir))
...
if __name__ == '__main__':
    ...
    matrixlexer = orio.tool.ply.lex.lex(debug=1,optimize=0) 
    ...
    for i in range(1, len(sys.argv)):
        ...
        theresult = parser.parse(s, lexer=matrixlexer, debug=0)
