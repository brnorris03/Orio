...
class Mlsearch(orio.main.tuner.search.search.Search):
    ...
    def __init__(self, params):
        ...
        if self.time_limit <= 0 and self.total_runs <= 0:
            err((
                            'orio.main.tuner.search.mlsearch.mlsearch: %s search requires either (both) the search time limit or (and) the ' +
    ...
    debug("Coordinates: %d\nUnevaluated coordinates: %d\nUnevaluated parameters: %d) " \
          % (len(coords), len(uneval_coords), len(uneval_params))
    ...
    if not math.isinf(mean_perf_cost):
		...
		while True:
			...
			debug("Shape X_train %s, Y_train %s, X_test %s" % (str(X_train.shape), str(Y_train.shape), str(X_test.shape)))
			...
			debug("Predicted values: %s" % str(np.sort(pred)[:batch_size]), obj=self)
			debug("Observed values: %s" % str(batch_cost), obj=self)
			debug("sruns=%d, total runs=%d" % str(sruns, self.total_runs), obj=self)
