...
class Randomsearch(orio.main.tuner.search.search.Search):
    ...
    def searchBestCoord(self, startCoord=None):
        ...
        for index in indices:
            ...
            pcost_items = sorted(perf_costs.items())
    ...
    def searchBestCoordOld(self, startCoord=None):
        ...
        while True:
            ...
            if not math.isinf(mean_perf_cost):
                sruns += 1
                info(
                    '(run %s) sruns: %s, fruns: %s, coordinate: %s, perf_params: %s, transform_time: %s, compile_time: %s, cost: %s' % (
                    runs + i, sruns, fruns, coord_val, perf_params, transform_time, compile_time, perf_cost))
